...
export default function NewRestockPage() {
  ...
  return (
    <section>
      ...
      <div className="mb-4">
        <label className="text-sm font-medium text-custom-black">Supplier</label>
        <select
          value={selectedSupplier}
          onChange={(e) => setSelectedSupplier(e.target.value)}
          className="w-full max-w-xs rounded-lg border border-bellflower-gray px-4 py-2 text-sm mt-1"
        >
          <option value="">Select supplier</option>
          {mockSuppliers.map((s) => (
            <option key={s.supplier_Id} value={s.company}>{s.company}</option>
          ))}
        </select>
      </div>
      ...
      <div className="flex gap-4 h-[calc(100vh-240px)]">
        ...
        <div className="flex-1 bg-white rounded-xl border border-bellflower-gray flex flex-col">
          <div className="p-4 border-b border-bellflower-gray">
            <h5>Restock Card</h5>
            <span className="text-xs text-vesper-gray">#RS-2026-NEW</span>
          </div>
          ...
          <div className="p-4 border-t border-bellflower-gray flex justify-between items-center">
            ...
            <div className="flex items-center gap-4">
              ...
              <button className="bg-river-green text-white px-4 py-2 rounded-lg text-sm font-semibold max-w-40 cursor-pointer">Save Restock</button>
            </div>
          </div>
        </div>
      </div>
